...
MODEL_PATH=/data/MY_USERNAME/hf_dl/Qwen2-VL-2B-Instruct

CURRENT_DATE_TIME=$(date  +"%Y%m%d_%H%M%S").log
CURRENT_LOGGING_FILE_NAME="verl_grpo_chart2code_mllm_qwen2-vl-2b-instruct-chartstruct4k-$CURRENT_DATE_TIME"
...
python -m verl.trainer.main_ppo \
    ...
    trainer.nnodes=1 \
    trainer.default_local_dir=targets/verl_grpo_chart2code_mllm_qwen2-vl-2b-instruct-chartstruct4k \
...
#
# 1. 快速后台启动
#    pyclean . && nohup bash chart2code-grpo-remain-Qwen2-VL-2B-Instruct-ChartStruct-4k.sh > /dev/null 2>&1 & 
# 
# 2. 一键后台 kill (发送 SIGTERM)
#    kill   $(ps aux | grep verl_grpo_chart2code_mllm_qwen2-vl-2b-instruct-chartstruct4k | grep -Ev 'grep --color=auto' | awk '{print $2}' | tr '\n' ' ')
#

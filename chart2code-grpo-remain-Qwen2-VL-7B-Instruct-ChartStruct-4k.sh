...
python -m verl.trainer.main_ppo \
    ...
    trainer.nnodes=1 \
    trainer.default_local_dir=targets/chart2code_mllm_remain-Qwen2-VL-7B-Instruct-ChartStruct-4k \
...
#
# 1. 快速后台启动
#    pyclean . && nohup bash chart2code-grpo-remain-Qwen2-VL-7B-Instruct-ChartStruct-4k.sh > /dev/null 2>&1 & 
# 
# 2. 一键后台 kill (发送 SIGTERM)
#    kill   $(ps aux | grep chart2code_mllm_remain-Qwen2-VL-7B-Instruct-ChartStruct-4k | grep -Ev 'grep --color=auto' | awk '{print $2}' | tr '\n' ' ')
#

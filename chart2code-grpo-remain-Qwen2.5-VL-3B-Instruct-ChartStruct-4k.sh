...
MODEL_PATH=/data/MY_USERNAME/hf_dl/Qwen2.5-VL-3B-Instruct

CURRENT_DATE_TIME=$(date  +"%Y%m%d_%H%M%S").log
CURRENT_LOGGING_FILE_NAME="verl_grpo_chart2code_mllm_qwen2.5-vl-3b-instruct-chartstruct4k-$CURRENT_DATE_TIME"

python -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo \
    data.train_files=/data/MY_USERNAME/dmc61-workspace/veRL-MLLM-GRPO/data-rl-parquet/chart2code_mllm-20260107_175738/chart2code_combo_ChartStruct_4k_train.parquet \
    data.val_files=/data/MY_USERNAME/dmc61-workspace/veRL-MLLM-GRPO/data-rl-parquet/chart2code_mllm-20260107_175738/chart2code_combo_ChartStruct_4k_train.parquet \
    ...
    trainer.nnodes=1 \
    trainer.default_local_dir=targets/verl_grpo_chart2code_mllm_qwen2.5-vl-3b-instruct-chartstruct4k \
...
#
# 1. 快速后台启动
#    pyclean . && nohup bash chart2code-grpo-remain-Qwen2.5-VL-3B-Instruct-ChartStruct-4k.sh > /dev/null 2>&1 & 
# 
# 2. 一键后台 kill (发送 SIGTERM)
#    kill   $(ps aux | grep verl_grpo_chart2code_mllm_qwen2.5-vl-3b-instruct-chartstruct4k | grep -Ev 'grep --color=auto' | awk '{print $2}' | tr '\n' ' ')
#

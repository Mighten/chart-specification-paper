...
MODEL_PATH=$HOME/hf_dl/Qwen2-VL-7B-Instruct
...
python -m verl.trainer.main_ppo \
    ...
    trainer.nnodes=1 \
    trainer.default_local_dir=targets/chart2code_mllm_qwen2_vl_7b_instruct_R1 \
...
#
# 1. 快速后台启动
#    pyclean . && nohup bash chart2code-mllm-grpo-Qwen2-VL-7B-Instruct.sh > /dev/null 2>&1 & 
# 
# 2. 一键后台 kill (发送 SIGTERM)
#    kill   $(ps aux | grep chart2code_mllm_ | grep -Ev 'grep --color=auto' | awk '{print $2}' | tr '\n' ' ')
#

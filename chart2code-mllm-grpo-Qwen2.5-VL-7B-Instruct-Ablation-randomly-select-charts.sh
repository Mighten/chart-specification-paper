...
python -m verl.trainer.main_ppo \
    ...
    trainer.nnodes=1 \
    trainer.default_local_dir=targets/chart2code_mllm_qwen2.5_vl_7b_instruct_ablation_randomly_select_charts \
...
#
# 1. 快速后台启动
#    pyclean . && nohup bash chart2code-mllm-grpo-Qwen2.5-VL-7B-Instruct-Ablation-randomly-select-charts.sh > /dev/null 2>&1 & 
# 
# 2. 一键后台 kill (发送 SIGTERM)
#    kill   $(ps aux | grep chart2code_mllm_qwen2.5_vl_7b_instruct_ablation_randomly_select_charts | grep -Ev 'grep --color=auto' | awk '{print $2}' | tr '\n' ' ')
#

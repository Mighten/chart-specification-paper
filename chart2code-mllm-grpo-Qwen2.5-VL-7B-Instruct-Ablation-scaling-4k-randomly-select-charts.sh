...
python -m verl.trainer.main_ppo \
    ...
    trainer.nnodes=1 \
    trainer.default_local_dir=targets/chart2code_mllm_qwen2.5_vl_7b_instruct_ablation_scaling_4k_randomly_select_charts \
...
#
# 1. 快速后台启动
#    pyclean . && nohup bash chart2code-mllm-grpo-Qwen2.5-VL-7B-Instruct-Ablation-scaling-4k-randomly-select-charts.sh > /dev/null 2>&1 & 
# 
# 2. 一键后台 kill (发送 SIGTERM)
#    kill   $(ps aux | grep chart2code_mllm_qwen2.5_vl_7b_instruct_ablation_scaling_4k_randomly_select_charts | grep -Ev 'grep --color=auto' | awk '{print $2}' | tr '\n' ' ')
#
...
# ---- 消融实验构造 4k 随机数据集 chart_type 分布比例
# the original train dataset has:  48342
# randomly selected training dataset: 4000
# radar ---> 197
# bar ---> 903
# line ---> 627
# pie ---> 218

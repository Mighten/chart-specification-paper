...
# 
# - 首先激活 Miniconda 虚拟环境
#   $ conda activate ~/miniconda3/envs/svgr1/
# - 选择合适显存空余容量的显卡
...
CURRENT_LOGGING_FILE_NAME="verl_grpo_chartqa_mllm_$CURRENT_DATE_TIME"

python -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo \
    data.train_files=/home/MY_USERNAME/dmc61-workspace/veRL-MLLM-GRPO/data-rl-parquet/chartqa_mllm-20250627_194023/chartqa_human_train.parquet \
    data.val_files=/home/MY_USERNAME/dmc61-workspace/veRL-MLLM-GRPO/data-rl-parquet/chartqa_mllm-20250627_194023/chartqa_human_val.parquet \
    ...
    trainer.project_name='chartqa_mllm' \
    trainer.experiment_name='beginner_guide' \
    trainer.n_gpus_per_node=8 \
    trainer.nnodes=1 \
    trainer.default_local_dir=targets/chartqa_mllm_qwen2.5_vl_7b_instruct \
...
# 0. 切换 *conda 虚拟环境：
#    conda activate svgr1                   # 61 机器
#    conda activate conda-env-logic-rl      # 171 机器
#
# 1. 快速后台启动
#    pyclean . && nohup bash chartqa-mllm-grpo.sh > /dev/null 2>&1 & 
# 
# 2. 一键后台 kill (发送 SIGTERM)
#    kill   $(ps aux | grep chartqa_mllm_qwen2.5_vl_7b_instruct | grep -Ev 'grep --color=auto' | awk '{print $2}' | tr '\n' ' ')
#

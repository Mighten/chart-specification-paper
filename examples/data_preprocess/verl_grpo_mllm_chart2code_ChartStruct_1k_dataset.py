...
def build_apache_parquet_for_mllm_grpo():
    ...
    parser.add_argument('--train_size', type=int, default=1002)
    ...
    train_parquet_filepath = Path(os.path.join(local_dir, f'chart2code_combo_ChartStruct_1k_train.parquet')).resolve().absolute().as_posix()
    train_dataset.to_parquet(train_parquet_filepath)

    print('')
    print(f'Output Training chart2code_combo_ChartStruct_1k_train Apache Parquet dataset filepath:', train_parquet_filepath)
    print('')

...
def build_apache_parquet_for_mllm_grpo():
    ...
    train_parquet_filepath = Path(os.path.join(local_dir, f'chart2code_combo_ChartStruct_4k_train.parquet')).resolve().absolute().as_posix()
    train_dataset.to_parquet(train_parquet_filepath)

    print('')
    print(f'Output Training chart2code_combo_ChartStruct_4k_train Apache Parquet dataset filepath:', train_parquet_filepath)
    print('')

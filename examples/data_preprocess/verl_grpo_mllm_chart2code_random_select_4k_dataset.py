...
def build_apache_parquet_for_mllm_grpo():
    ...
    raw_train_dataset = raw_train_dataset.select(range(args.train_size))
    print('randomly selected training dataset:', len(raw_train_dataset))
    ...
    train_parquet_filepath = Path(os.path.join(local_dir, f'chart2code_combo_4k_train.parquet')).resolve().absolute().as_posix()
    train_dataset.to_parquet(train_parquet_filepath)

    print('')
    print(f'Output Training chart2code-combo-4k Apache Parquet dataset filepath:', train_parquet_filepath)
    print('')
